...
#' to the untracked c_historical_RS_data_collation/in/ folder
#'
...
#' c_historical_RS_data_collation/in/ folder
#' 
...
download_csvs_from_drive <- function(drive_folder_name, version_identifier) {
  ...
  if(!dir.exists(file.path("c_historical_RS_data_collation/in/", 
                           version_identifier))) {
    dir.create(file.path("c_historical_RS_data_collation/in/", 
                         version_identifier))
  }
  walk2(.x = dribble_files$id,
        ...
        .f = function(.x, .y) {
          try(drive_download(file = .x,
                         path = file.path("c_historical_RS_data_collation/in/", 
                                          version_identifier,
                                          .y),
