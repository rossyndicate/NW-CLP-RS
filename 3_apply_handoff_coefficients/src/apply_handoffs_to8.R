apply_handoffs_to8 <- function(coefficients, data_filepath) {
  ...
  data_out <- data %>% 
    mutate(med_Blue_corr8 = med_Blue_intercept + med_Blue_B1*med_Blue + med_Blue_B2*med_Blue^2,
    ...
    mutate(med_Blue_corr8 = ifelse(mission == 'LANDSAT_8', med_Blue, med_Blue_corr8),
    ...
    mutate(flag_Aerosol_8 = ifelse(med_Blue < med_Blue_max_in_val & 
  ...
  data_out <- data_out %>% select(-c(med_Aerosol_intercept:med_Swir2_B2))
